...
$allCommands = 'cd /var/www/my-fullstack-app && git stash push -m backup-$(date +%Y%m%d_%H%M%S) 2>/dev/null || true && git fetch gitee main && git reset --hard gitee/main && cd backend && source ../venv/bin/activate && pip install --upgrade pip --quiet && pip install -r requirements.txt --quiet && python init_db.py 2>&1 || echo "DB init done" && cd ../frontend && export NODE_OPTIONS="--max-old-space-size=2048" && npm install --silent && npm run build && systemctl daemon-reload && systemctl restart my-fullstack-app && sleep 5 && systemctl restart nginx && sleep 2 && echo "=== Service Status ===" && systemctl status my-fullstack-app --no-pager -l | head -15 && echo "" && echo "=== API Health Check ===" && curl -s http://127.0.0.1:8000/api/health && echo "" && echo "Sync completed!"'
