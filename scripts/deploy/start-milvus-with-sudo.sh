#!/bin/bash
# Milvus 启动脚本（使用 sudo，解决权限问题）

set -e

echo "=========================================="
echo "启动 Milvus 和 Attu（使用 sudo）"
echo "=========================================="
echo ""
...
# 步骤 2: 手动拉取镜像（使用 sudo，20分钟超时）
echo "步骤 2: 拉取镜像（使用 sudo）..."
IMAGES=(
...
for IMAGE in "${IMAGES[@]}"; do
    ...
    while [ $RETRY -lt $MAX_RETRIES ]; do
        ...
        if sudo timeout 1200 docker pull "$IMAGE" 2>&1; then
            echo "✓ $IMAGE 拉取成功！"
            SUCCESS=true
            break
        else
