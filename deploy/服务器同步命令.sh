#!/bin/bash
# 在阿里云服务器上直接执行的同步命令
# 使用方法: 复制以下命令到服务器执行，或执行: bash deploy/服务器同步命令.sh
...
KEY_FILES=(
    ...
    "deploy/sync-on-server-complete.sh"
    "deploy/fix-502-error.sh"
)
...
if [ -f "/etc/systemd/system/my-fullstack-app.service" ]; then
    echo "✓ systemd 服务文件存在"
    # 检查关键配置
    if grep -q "ExecStart" /etc/systemd/system/my-fullstack-app.service; then
        echo "✓ 服务配置正确"
    else
        echo "⚠ 服务配置可能有问题"
    fi
